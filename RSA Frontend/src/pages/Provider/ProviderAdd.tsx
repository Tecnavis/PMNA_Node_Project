...
const ProviderAdd: React.FC = () => {
    ...
    const [selectedServiceTypes, setSelectedServiceTypes] = useState<{ id: string; name: string; overRideCharge: string; kmForBasicAmount: string; basicAmount: string; vehicleNumber: string }[]>([]);
    const [companyName, setCompanyName] = useState<string>('');
    const [baseLocation, setBaseLocation] = useState<string>('');
    const [idNumber, setIdNumber] = useState<string>('');
    const [creditAmountLimit, setCreditAmountLimit] = useState<number | null>(null);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (validate()) {
            ...
            formData.append('creditAmountLimit', creditAmountLimit?.toString() || '');
            formData.append('phone', phone);
            formData.append('personalPhoneNumber', personalPhoneNumber);
            formData.append('password', password);
            formData.append('serviceDetails', JSON.stringify(selectedServiceTypes));
            if (profileImage) {
            ...
            try {
                const response = await axios.post(`${backendUrl}/provider`, formData, {
                    headers: {
                ...
                navigate('/users/provider');
                // Optionally, reset the form here
            } catch (error: unknown) {
                if (error instanceof AxiosError) {
                    // Error is an Axios error
                    console.error('Error creating provider:', error.response?.data?.message || error.message);
    ...
    useEffect(() => {
        ...
        const fetchProviderById = async () => {
            try {
                const response = await axios.get(`${backendUrl}/provider/${uid}`);
                ...
                setBaseLocation(data.baseLocation?._id || '');
                setCreditAmountLimit(data.creditAmountLimit || '');
                ...
                if (data.serviceDetails?.length) {
                    ...
                    setSelectedServiceTypes(formattedServiceDetails);
                }
            } catch (error) {
    ...
    const handleUpdate = async (e: React.FormEvent) => {
        ...
        if (validate()) {
            ...
            try {
                const response = await axios.put(`${backendUrl}/provider/${uid}`, formData, {
                    headers: {
                        'Content-Type': 'multipart/form-data',
                    },
                });
                ...
                Swal.fire({
                    icon: 'success',
                    title: 'Updated provider successfully',
                    toast: true,
                    position: 'top',
                    showConfirmButton: false,
                    timer: 3000,
    ...
    // validation for add or update provider
    ...
    const validate = (): boolean => {
        ...
        if (typeof baseLocation === 'string' && !baseLocation.trim()) {
            formErrors.baseLocation = 'Base location required';
        }
        ...
        selectedServiceTypes.forEach((service) => {
                ...
                <div className="flex flex-col sm:flex-row">
                    ...
                    <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            {errors.baseLocation && <p className="text-red-500">{errors.baseLocation}</p>}
                        </div>
                        ...
                        <div>
                            <label htmlFor="creditAmountLimit">Credit Amount Limit</label>
                            <input
                                id="creditAmountLimit"
                                type="number"
                                placeholder="Enter Credit Amount Limit"
                                className={`${styles.formInput} form-input`}
                                value={creditAmountLimit !== null ? creditAmountLimit : ''}
                                onChange={(e) => {
                                    const value = e.target.value;
                                    setCreditAmountLimit(value === '' ? null : Number(value));
                                }}
                            />
                        </div>
                                    ...
                                    {serviceTypes?.map((type) => (
                                        <label key={type._id} className="flex items-center space-x-2">
                                            <input
                                                ...
                                                checked={selectedServiceTypes.some((item) => item.id === type._id)}
                                            />
