...
const ProviderAdd: React.FC = () => {
    ...
    const [creditLimitAmount, setCreditLimitAmount] = useState<number | null>(null);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (validate()) {
            ...
            formData.append('creditLimitAmount', creditLimitAmount?.toString() || '');
            ...
            try {
                const response = await axios.post(`${backendUrl}/company`, formData, {
                    headers: {
    ...
    useEffect(() => {
        ...
        const fetchCompanyById = async () => {
            try {
                const response = await axios.get(`${backendUrl}/company/${uid}`);
                ...
                setPhone(data.phone || '');
                setCreditLimitAmount(data.creditLimitAmount || '');
    ...
    const handleUpdate = async (e: React.FormEvent) => {
        ...
        if (validate()) {
            ...
            try {
                const response = await axios.put(`${backendUrl}/company/${uid}`, formData, {
                    headers: {
                        'Content-Type': 'multipart/form-data',
                    },
                });
            ...
            <form className="border border-[#ebedf2] dark:border-[#191e3a] rounded-md p-4 mb-5 bg-white dark:bg-black">
                ...
                <div className="flex flex-col sm:flex-row">
                    ...
                    <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div>
                            <label htmlFor="creditLimitAmount">Credit Amount Limit</label>
                            <input
                                id="creditLimitAmount"
                                type="number"
                                placeholder="Enter Credit Amount Limit"
                                className={`${styles.formInput} form-input`}
                                value={creditLimitAmount !== null ? creditLimitAmount : ''}
                                onChange={(e) => {
                                    const value = e.target.value;
                                    setCreditLimitAmount(value === '' ? null : Number(value));
                                }}
                            />
                        </div>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                                    {serviceTypes.map((type) => (
                                        <label key={type._id} className="flex items-center space-x-2">
                                            <input type="checkbox" value={type._id} onChange={() => handleServiceTypeToggle(type)} checked={selectedVehicle.some((item) => item.id === type._id)} />
